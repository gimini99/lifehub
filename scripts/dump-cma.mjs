// Dump derived CMA from historical returns for a sanity check.
// Usage: node scripts/dump-cma.mjs

const ASSET_CLASSES = ["USStock","IntlDeveloped","IntlEmerging","Bond","ShortBond","HighYieldBond","Cash","Gold","Sector","Other"];

// Inline copy of HISTORICAL_ROWS — duplicate of historicalReturns.ts for standalone run.
const HISTORY = [
...
function meanStdev(xs) {
  ...
  return { mean, stdev: Math.sqrt(v / (n - 1)), n };
}
function corr(a, b) {
  const xs = [], ys = [];
  for (const row of HISTORY) {
    if (row[a] == null || row[b] == null) continue;
    xs.push(Math.log(1 + row[a]));
    ys.push(Math.log(1 + row[b]));
  }
...
console.log("Empirical CMA from 1973-2023 history:\n");
console.log("Class            obs   mu(log)  sigma(log)  E[r]    sigma");
for (const c of ASSET_CLASSES) {
  const lrs = logReturns(c);
  if (lrs.length < 15) {
    console.log(`  ${c.padEnd(15)} ${String(lrs.length).padStart(3)}    (fallback)`);
    continue;
  }
  const { mean, stdev, n } = meanStdev(lrs);
  const er = Math.exp(mean) - 1;
  console.log(`  ${c.padEnd(15)} ${String(n).padStart(3)}   ${(mean * 100).toFixed(2).padStart(6)}%  ${(stdev * 100).toFixed(2).padStart(6)}%   ${(er * 100).toFixed(2).padStart(6)}%  ${(stdev * 100).toFixed(2).padStart(6)}%`);
}

console.log("\nCorrelation matrix (log-returns):\n");
process.stdout.write("                ");
for (const c of ASSET_CLASSES) process.stdout.write(c.slice(0, 6).padStart(7));
console.log();
for (const a of ASSET_CLASSES) {
  process.stdout.write("  " + a.padEnd(14));
  for (const b of ASSET_CLASSES) {
    if (a === b) { process.stdout.write("   1.00"); continue; }
    const r = corr(a, b);
    process.stdout.write(r == null ? "    n/a" : (r >= 0 ? " " : "") + r.toFixed(2).padStart(6));
  }
  console.log();
}
